...
struct _vqueue{
	int shmem_fd; uint32_t lock_id;
	struct{
		sem_t sema4;
		_Atomic uint64_t next_msg;
...
vqueue_t vqueue_open(const char* path){
	...
	q.lock_id = atomic_fetch_add_explicit(&q.header->open_counter, 1, memory_order_relaxed);
	struct flock l = { .l_type = F_WRLCK, .l_whence = SEEK_SET, .l_start = q.lock_id, .l_len = 1 };
...
void vqueue_post(vqueue_t q, const vqueue_block_t block){
	//sem_post(&q.header->sema4);
}

vqueue_block_t vqueue_wait(vqueue_t q){
	//sem_wait(&q.header->sema4);
}
