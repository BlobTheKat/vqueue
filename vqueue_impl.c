...
static inline void _vqueue_resize_mapping(struct _vqueue_mapping* ctx, uint64_t newsize){
	...
	struct _vqueue_shmem_region* new_map = _vqueue_mmap(q->_vqueue_shmem_fd, 0, newsize);
...
bool vqueue_open(vqueue_t* q, const char* name, size_t name_sz){
	...
	struct _vqueue_fd_map_entry* e = _vqueue_fd_map_find(name, name_sz);
	if(!e) return false;
	if(e->fd >= 0) q->_vqueue_shmem_fd = e->fd;
	else{
#endif
	...
	path[name_sz+9] = '\0';
	q->_vqueue_shmem_fd = open(path, O_RDWR | O_CREAT, 0666);
	if(q->_vqueue_shmem_fd < 0){
		if(mkdir("/tmp/shm/", 01777) && errno != EEXIST) return false;
		q->_vqueue_shmem_fd = open(path, O_RDWR | O_CREAT, 0666);
		if(q->_vqueue_shmem_fd < 0) return false;
	}
#else
	char* path = alloca(NAME_MAX + 2);
	memcpy(path+1, name, name_sz);
	path[0] = '/'; path[name_sz+1] = '\0';
	q->_vqueue_shmem_fd = shm_open(path, O_RDWR | O_CREAT, 0666);
#endif
#ifdef _VQUEUE_SHARED_FDS
		e->fd = q->_vqueue_shmem_fd;
	}
	...
	struct _vqueue_shmem_region* mapping = _vqueue_mmap(q->_vqueue_shmem_fd, 0, sz);
	if(mapping == (void*)-1){
		// Init perhaps
		struct flock l = { .l_type = F_WRLCK, .l_whence = SEEK_SET, .l_start = q->aid, .l_len = 1 };
		fcntl(q->_vqueue_shmem_fd, LOCK_EX);
	}
	...
	obtain_id: {
		...
		struct flock l = { .l_type = F_WRLCK, .l_whence = SEEK_SET, .l_start = q->aid, .l_len = 1 };
		if(fcntl(q->_vqueue_shmem_fd, F_SETLK, &l)){
			if(errno == EACCES || errno == EAGAIN) goto obtain_id;
			munmap(mapping, sz);
			close(q->_vqueue_shmem_fd);
			return false;
		}
	}
...
#ifdef _VQUEUE_SHARED_FDS
	_vqueue_fd_map_finish(q->e);
#else
	close(q->_vqueue_shmem_fd); // clears locks
#endif
}
		...
		if(vm == _VQUEUE_PTR_INVALID){
			if(!(*v_ >> 48)){
				...
				if(!atomic_compare_exchange_strong_explicit(&ctx->data->left, &expected_left, 0, memory_order_relaxed, memory_order_relaxed)){
					atomic_compare_exchange_strong_explicit(ctx->data8+rp, v_, rinit, memory_order_relaxed, memory_order_relaxed);
					*v_ = rinit;
					goto retry;
				}
			...
			atomic_compare_exchange_strong_explicit(ctx->data8+rp, &v_, 0, memory_order_release, memory_order_relaxed);
		}
...
static uint8_t* _vqueue_try_alloc(struct _vqueue_mapping* ctx, _Atomic uint64_t* rp, _Atomic uint64_t* rip, uint64_t blocks, uint64_t size, uint64_t v){
	...
	if(v <= left && v + blocks > left){
		...
		if(v < left){
			...
			uint64_t v2 = v;
			atomic_compare_exchange_strong_explicit(rip, &v2, left, memory_order_relaxed, memory_order_relaxed);
		}
		uint8_t n = _vqueue_protect(ctx, 0);
		uint64_t mark = _VQUEUE_PTR_INVALID|n<<40;
		if(!atomic_compare_exchange_strong_explicit(rp, &v, mark, memory_order_acquire, memory_order_relaxed))
			goto retry;

		if(!atomic_compare_exchange_strong_explicit(&ctx->data->left, &left, 0, memory_order_release, memory_order_relaxed)){
			atomic_compare_exchange_explicit(rp, &mark, v, memory_order_relaxed);
			_vqueue_unprotect(ctx, 0, n);
			goto retry2;
		}
		atomic_store_explicit(rip, 0, memory_order_release);

		atomic_compare_exchange_strong_explicit(rp, &mark, 0, memory_order_release, memory_order_relaxed);
		_vqueue_unprotect(ctx, 0, n);
		return 0;
	}
...
void vqueue_post(vqueue_t* q, vqueue_block_t block){
	...
	uint64_t ptr = atomic_load_explicit(&ctx.data->tail, memory_order_relaxed), vo;
	retry:
	ptr &= 0xFFFFFFFFFF;
...
vqueue_block_t vqueue_wait(vqueue_t* q){
	...
	find_next:
	if((ptr&0xFFFFFFFFFF) == _VQUEUE_PTR_INVALID){
		// No messages. Block
		_vqueue_unprotect(&ctx, ptr, n);
...
static inline bool _vqueue_trim(struct _vqueue_mapping* ctx, uint64_t left, bool strict){
	...
	while(true){
		...
		if(strict && aid != -1u){
			...
			if(!fcntl(ctx->q->_vqueue_shmem_fd, F_GETLK, &l) && l.l_type == F_UNLCK) aid = -1u;
		}
		if(aid == -1u && _vqueue_check(ctx, left)){
			...
			recheck:
			if(next == rmax){
				if(!atomic_compare_exchange_strong_explicit(ctx->data8+right, &rmax, _VQUEUE_PTR_INVALID|256<<40, memory_order_relaxed, memory_order_relaxed)){
					// update max
				...
				left = 0;
				rmax = _VQUEUE_PTR_INVALID|256<<40;
